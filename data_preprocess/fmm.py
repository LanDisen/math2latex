...
input_label_dir = '../dataset/datasets_no_test/train/labels/'
output_label_dir = '../dataset/data/train/labels/'
...
# 以下为新增代码
def tokenize(input_label_dir, output_label_dir, have_chinese=False):
    '''
    封装分词预处理操作
    Args:
        have_chinese: 是否有中文（文字混合识别有中文）
    '''
    if not os.path.exists(output_label_dir):
        os.mkdir(output_label_dir)
    label_name_list = os.listdir(input_label_dir)
    ...
    for label_name in label_name_list:
        ...
        # print(new_content)
        # have_chinese = False # 是否有中文（文字混合识别有）
